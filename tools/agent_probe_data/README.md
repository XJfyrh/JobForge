...
审查修复后本目录共 32 项确定性测试，覆盖所有推理阶段完成未知时先落盘并停止、响应未完成与已完成坏输出的区别、可选 `/api/ps` 失败不能抹除证据，以及当前案例三个工具都被读取且提交三个不同的实际引用。修正版没有重新调用模型；历史模型结果和当时源码 hash 保持不变，见[版本与验证说明](../../docs/evidence/agent-v3-model-probe-2026-09-16-summary.md#审查后的探针修复)。
