...
第6条的 `repeated_tool_call` 固定诊断写入现有安全日志，关联 Run ID；公开失败码仍为 `MODEL_PROTOCOL_ERROR`，Steps 保留导致拒绝的决定，Calls 可核对没有第二次工具派发。原定“复用现有失败消息”经核对不适用：当前 FailAttempt RPC 仅接收错误码，消息由 domain 派生。为避免只为诊断新增 RPC/数据库字段，采用上述现有通道；重复拒绝、扣额和终结语义不变。query 规范化明确只移除首尾 ASCII 空格、tab、CR、LF，Go/Python一致。
