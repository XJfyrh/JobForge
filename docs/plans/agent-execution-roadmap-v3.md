...
- 规划日期：2026-09-16；2026-09-17 当前状态：**S0 已合并，S1 已有业务/运行时/审计实现，但完整真实验收未完成；S2～S5 未开始**。修复后新批次 14 案方案完成、DEV-015 中断、25 案未尝试，因 `CHAT_USAGE_UNKNOWN` 停止；[PR #51](https://github.com/XJfyrh/JobForge/pull/51) 保持 Draft。详见[实施记录](../agent-v3-progress.md)与[最新阻塞证据](../evidence/agent-v3-s1-closeout-2026-09-17.md)，以下仍为完整路线规划，不表示各阶段均已交付。
- 规划前提：按用户要求，不以历史实现、旧接口兼容和既有验收编号约束新版本设计。
- 2026-09-16 调整：云端 chat 模型承担主线开发、正式评测和演示；本地 chat 是可选扩展，不再要求先通过本地模型验收。此前本地试验结果作为历史证据保留。
...
维护者于2026-09-16指定云端优先支持DeepSeek，取代此前百炼首选候选。当前账号的[模型列表](https://api-docs.deepseek.com/api/list-models/)实际返回 `deepseek-flash` 与 `deepseek-v4-pro`；首轮拟采用Flash非思考模式及预注册工具，具体profile和额度须在适配器中落实，不能把模型别名当作不可变快照。只读接口检查不代表工具选择、参数、反馈利用、终止或业务质量通过，见[调查与阻塞记录](../evidence/agent-v3-deepseek-and-blocker-2026-09-16.md)。云端适配和最小持久调用账本计划在S1并行开发，凭据与费用明确后运行有界接入验证，S2再完成真实业务Agent验收。无需等待本地 chat 成功或完整S2系统建成后才开发适配器；此前按维护者要求因Docker故障暂停；同日重启后已通过[恢复验收](../evidence/agent-v3-docker-recovery-2026-09-16.md)，环境阻塞解除。
...
以下是S0契约中的初始资源约束，不是已实现或验收能力；实际收费额度仍须随具体profile落实：
...
云端预算上限 B 尚未确定。探测、日常开发、正式评测、收费query embedding与故障测试都计入B；定价和隐藏计费项无法得到保守上界的模式不纳入本版硬预算承诺。已有[首批最多27请求、最高1元的费用提案](../evidence/agent-v3-model-probe-2026-09-16.md#远程备选与授权前缺口)，尚未授权。缺少凭据或预算只阻挡实际收费调用，不阻挡云端适配器、调用账本、业务工具、持久化及确定性测试；真实云端验收保持未完成，不能用mock通过替代。
...
| 阶段 | 时间 | 交付与退出条件 |
|---|---:|---|
| S0 产品契约与关键试验 | 2～3 日 | 新版PRD、必要ADR；Run/步骤/审批/副作用协议；Python进程试验；云端候选、profile要求和费用方案。契约/进程交付可独立评审，本地chat成功不作为退出条件；云端实际调用未验收单列 |
| S1 真实业务工具与基线 | 4～5 日 | 合成工单/订单/物流/政策；真实HTTP业务服务、pgvector索引、三个读工具、合理固定流程；并行实现单一云端适配及最小持久调用账本；开发集和计分器可核对 |
...
S1 即开始开发SDK/API契约和基本运行详情；云端适配、最小持久费用预留与调用账本同期开工，后续扩展为S2的Run/tenant/批次治理，不另建一次性绕过额度的探测通道。任何收费请求之前必须落实相应额度约束。云端凭据或费用未就绪时继续这些工程工作，不能把S2真实Agent验收标为完成。

时间不足时按顺序裁剪：本地chat与第二云端后端、页面美化、检索优化、额外场景与重复实验。保留单场景真实工具循环、步骤恢复、一个审批点、一个幂等写入、故障证据和基本评测；这些共同构成本版区别于普通聊天/RAG 演示的主线。
...
**维护者已确认售后工单场景、Go/Python 分工，以及步骤恢复和一个审批写入闭环的主线。** S0契约与探针已随PR #35合并，Docker阻塞已解除；S1～S5尚未实现。详细契约、探针和后续实现状态以[PRD v0.7](../product/JobForge_PRD_v0.7.md)及[实施记录](../agent-v3-progress.md)为准；路线获确认不代表功能验收完成。
