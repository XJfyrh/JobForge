...
## v0.4 持久业务幂等与真实进程崩溃收官

> 测量日期：2026-08-17；代码为合并前 PR 1～3 的堆叠树（原始实现提交 `4377812`、测试提交 `fa870fa`，后续跨平台 Wait 修正 `2eb7a3b` 不改变被测路径）；等价代码已通过 PR #24/#25 squash 合入 `main`。硬件、Go、PostgreSQL、Docker 与实现前一致；应用服务停止，仅运行测试/基准进程。

### Claim 五轮前后比较

实现完成并通过完整 integration/scale schema 重置后，以与实现前完全相同的命令执行：

| 轮次 | ns/op | B/op | allocs/op |
|---:|---:|---:|---:|
| 1 | 5,658,688 | 6,887 | 91 |
| 2 | 7,218,370 | 6,888 | 91 |
| 3 | 6,360,218 | 6,886 | 91 |
| 4 | 6,679,887 | 6,887 | 91 |
| 5 | 7,682,005 | 6,887 | 91 |
| **中位数** | **6,679,887** | **6,887** | **91** |

| 口径 | 实现前中位数 | 实现后中位数 | 变化 | 15% 门禁 |
|---|---:|---:|---:|---|
| BenchmarkClaim | 7.233324ms/op | 6.679887ms/op | **-7.65%（改善）** | PASS |
| B/op | 6,888 | 6,887 | -0.01% | PASS |
| allocs/op | 91 | 91 | 0 | PASS |

五轮之间仍受同库行数累积影响；实现前后使用同一容器与命令，但中间的 integration/scale 套件按设计重置过 schema，因此不是逐页物理数据副本对照。新增 0018 不在 Claim SQL 路径，当前中位数没有回退。历史 W4 `4.171091ms/op` 绝对值对比仍为 +60.1%，继续标为**未通过**，不得用 v0.4 当前基线覆盖。

### 完整 scale 与 20,000-job Claim

`go test -tags scale -count=1 -v -timeout 60m ./tests/scale/` 完整通过（342.406s）：

| 口径 | M4 当前基线 | v0.4 实测 | 变化 | 结果 |
|---|---:|---:|---:|---|
| 20k Claim p50 | 88.716ms | 95.7573ms | +7.9% | PASS（<15%） |
| 20k Claim p95 | 102.177ms | 115.1454ms | +12.7% | PASS（<15%） |
| Promote p50 / p95 | — | 24.3975 / 88.2637ms | 只报告 | PASS |
| NFR-302 throughput | floor 1,800 events/sec | 2,196 events/sec | +22.0% 高于 floor | PASS |
| Event publish lag p95 | ≤2s | 156.87ms | — | PASS |

同一命令还完成 AT-13 100 轮真实 Worker 进程 kill、AT-14 10,000 持久效果重投、NFR-306 与多租户公平性；详见 [reliability-report.md](reliability-report.md)。

### 同参数 e2e

`go run ./benchmarks/e2e -jobs=100 -workers=4`：

| 指标 | M4 同机结果 | v0.4 实测 | 变化 | 结果 |
|---|---:|---:|---:|---|
| Submit 吞吐 | 342.21 jobs/sec | 313.36 jobs/sec | -8.4% | PASS（回退 <15%） |
| Process 吞吐 | 380.95 jobs/sec | 354.49 jobs/sec | -6.9% | PASS（回退 <15%） |
| p50 | — | 10.3658ms | — | 只报告 |
| p95 | 12.8372ms | 13.4332ms | +4.6% | PASS（恶化 <15%） |
| p99 | — | 31.8768ms | — | 只报告 |
