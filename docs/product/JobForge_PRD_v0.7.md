...
- 日期：2026-09-16。
- 状态：维护者已确认[路线 v3](../plans/agent-execution-roadmap-v3.md)，本详细契约进入 S0 评审；功能尚未验收。
...
| 范围 | 初始约束 |
|---|---|
| 主后端 | 一个真实 chat 后端 + 一个固定 embedding profile；本地优先 |
...
远程供应商和预算 B 仍未选择；在任何收费请求（包括探测）前必须确认 B、计价版本和可保守上界的模式。现阶段仅授权免费本地试验。固定模型版本、digest（可得时）、参数、执行器与工具 Schema 进入 profile。
...
S0 交付本 PRD、三项 ADR、可复现执行器探针、本地模型探针和结果，明确正式实现是否可采用推荐方案。S0 不声称业务 Run、checkpoint、审批、预算表或写工具已经实现。
