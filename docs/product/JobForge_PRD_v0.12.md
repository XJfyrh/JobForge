...
- 基线：[PRD v0.10](JobForge_PRD_v0.10.md)、[v0.11](JobForge_PRD_v0.11.md)、ADR-0017～0019；对应决策为 [ADR-0020](../adr/0020-provider-audit-and-batch-stop.md)。
- 对应代码核对：C3b 已通过 PR #46 合并，起点 `0829a4c`。本稿定义下一合同，不修改已合并实现。
...
本稿建议为**新首批审计 profile**增加一条明确的保守运行政策：任一 chat 的计量最终为 unknown、供应商身份/非思考模式不兼容、报告冲突或持久确认不确定，均停止本批次后续收费调用。unknown 仍是合法的账本事实，不被篡改成零费用或 measurement anomaly。唯一纠正再次校验失败时，若报告、正常结算、普通 rejected 观察与原步骤的终态 MODEL_PROTOCOL_ERROR 都已持久，允许下一案例，失败仍计入分母；尺寸停止没有该普通观察，仍停批。该政策比旧 profile 的“保留 hold 后在额度允许时可继续”更窄；不追溯改变旧 profile 的晚到计量权限。
...
| A-04 | 同事务和幂等 | 实际 PG 中，首次 coherent report 的审计、符合定价资格的 usage 结算以及必要批次冻结同事务；同报告重放不重复扣费；不同报告不覆盖首次事实，冲突有持久停批且不能 ACK 成功；第二份冲突报告即使计数超界也不能改首份计量或新触发三层冻结 |
...
| A-06 | 停批职责 | Python 停止当前执行；Go 锁存停止 Claim/许可/纠正并清理；控制面持久冻结对应 batch，并以已预留 chat 的报告/普通观察/匹配步骤提交或窄终态失败事实阻断未确认时的后续许可；SDK 驱动停止提交下一例。实际验证二次校验失败可继续下一例，尺寸停止、取消、失权及各错配事实均不能解锁。重启或另一 driver 不能绕过持久 guard；计量异常三层冻结与审计原因仅 batch 冻结明确区分；未确认不能宣称已冻结 |
