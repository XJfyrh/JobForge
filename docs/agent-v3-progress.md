...
| 阶段 | 状态 | 当前证据 |
|---|---|---|
| S0 契约与关键试验 | 工程检查点已交付，详细决策评审中 | PRD v0.7、ADR-0013～0015；三项审查问题已修复，执行器10类真进程/race通过；云端候选及费用方案已记录，本地成功不再是退出条件 |
...
- 云端主线已确定，百炼候选待实际接入；S1并行开发云端适配、最小持久调用账本、业务服务和检索。账号/凭据/预算缺口只阻挡收费请求，S2真实Agent验收仍须完成。[本地历史结论及费用提案](evidence/agent-v3-model-probe-2026-09-16-summary.md)不代表云端通过或费用授权。
...
| 全仓Go集成/race、PR CI | S0代码提交通过 | [56411c6 CI](https://github.com/XJfyrh/JobForge/actions/runs/35004928363)六个job全部通过，含真实PG、Redis和SDK HTTP契约；后续文档修订不冒充新模型运行，最新状态见[PR checks](https://github.com/XJfyrh/JobForge/pull/35/checks) |
| 云端主chat | 未调用、未验收 | 候选/费用方案已记录；适配与账本可开发，首次收费请求前须落实凭据和额度 |
...
云端候选已记录，实际账号/凭据和费用B待落实，没有收费调用。S0的模型工具fixture只检查模型协议，不作为真实业务验收。详细ADR仍为Proposed，契约PR按评审和CI结果推进，不再因本地chat失败而阻挡；S2及整体目标仍需真实云端业务验收。此调整修改后续路线，不把既有失败改成通过。
