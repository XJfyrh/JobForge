...
| 阶段 | 状态 | 当前证据 |
|---|---|---|
| S0 契约与关键试验 | 草稿PR评审中，后续补丁待完整验证 | PRD v0.7、ADR-0013～0015；旧提交执行器10类真进程/race通过；新独立审查的引用与stderr问题已修复，当前补丁仅完成确定性/WSL定向验证，固定Docker复验受阻 |
| S1 业务与基线 | 暂停，未实现 | DeepSeek只读接入检查通过；Docker启动阻塞，无真实新工单工具/pgvector验收 |
...
| Linux执行器真进程/race | 旧提交通过；当前补丁完整复验无法运行 | 旧提交固定镜像、1CPU/256MiB/64PID、10个真实场景，无skip；[历史记录](../tools/executorprobe/acceptance-2026-09-16.txt)。当前stderr补丁[WSL定向回归通过](../tools/executorprobe/stderr-eof-regression-2026-09-16.txt)，未启用race；Docker启动失败 |
...
| 全仓Go集成/race、PR CI | e6b0038通过；当前补丁待CI | [e6b0038 CI](https://github.com/XJfyrh/JobForge/actions/runs/35006231149)六个job全部通过，含真实PG、Redis和SDK HTTP契约；不冒充当前补丁验证，最新状态见[PR checks](https://github.com/XJfyrh/JobForge/pull/35/checks) |
...
审查后的模型探针统一unknown停止和证据先落盘，修复仅经确定性回归测试；原始真实模型报告仍对应18a9302基线，没有替换为修复后结果。此前执行器源码曾重新进行真进程race；本轮追加stderr补丁后，仅完成WSL确定性定向回归，固定Docker/race受阻。本轮没有运行Windows宿主全仓集成；旧提交真实数据库全仓race在上述Linux CI执行。当前没有修改核心热路径，未新增性能门禁结论。

DeepSeek账号/凭据只读检查已通过，推理费用上限及调用账本尚未落地，没有推理调用。S0的模型工具fixture只检查模型协议，不作为真实业务验收。详细ADR仍为Proposed，契约PR按评审和CI结果推进，不再因本地chat失败而阻挡；当前仍保持草稿、未合并。S2及整体目标仍需真实云端业务验收。此调整修改后续路线，不把既有失败改成通过。
